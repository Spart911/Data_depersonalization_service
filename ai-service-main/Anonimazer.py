...
def read_and_create_list(file_path):
    try:
        ...
        # Удаляем повторяющиеся слова, сохраняя порядок
...
if __name__ == '__main__':
    file_path_exeption = 'src\\ai-service-main\\exeption.txt'
    ...
    src_path = os.path.abspath(os.path.join(current_script_path, '..', '..'))

    # Формирование абсолютных путей на основе текущего пути скрипта
    absolute_file_path_exeption = os.path.join(src_path, file_path_exeption)
    absolute_directory_path = os.path.abspath(os.path.join(src_path, 'src', 'input'))
    absolute_end_path = os.path.abspath(os.path.join(src_path, 'src', 'output'))

    List = []
    result_list = []
    ListADD = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября", "октября",
               "ноября", "декабря", "мужской", "женский", "Мужской", "Женский"]
    ListExeption = read_and_create_list(absolute_file_path_exeption)

    files = os.listdir(absolute_directory_path)
    if not files:
        print(f'В каталоге {absolute_directory_path} нет файлов.')
        sys.exit()
    else:
    ...
    if extension.lower() == '.pdf':
        ...
        docx_path = os.path.join(absolute_end_path, os.path.splitext(name_file)[0] + '.docx')
        pdf_path = os.path.join(absolute_end_path, os.path.splitext(name_file)[0] + '.pdf')
